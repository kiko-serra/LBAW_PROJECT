<div id="editUserModal" class="hidden fixed flex justify-center items-center overflow-y-auto overflow-x-hidden h-screen w-screen top-0 right-0 left-0 z-50">
    ...
    <div class="relative w-fit h-fit bg-white opacity-100 flex flex-col justify-start items-center p-4 rounded-lg mt-4">
        <Form method="POST" id="edit_profile" action={{ route('profile.edit', ['id' => $user->id_account]) }}>
            ...
            <input id="editUserName" type="text" name="name" <?php echo 'data-default-name="' . $user->name . '"'; ?> value={{ $user->name }} maxlength="32">
            @if ($errors->has('name'))
            <span class="error">
                {{ $errors->first('name') }}
            </span>
            ...
            <div class="flex flex-row justify-start">
                <input id="editUserPrivacyPrivate" type="radio" name="privacy" value="private" id="profilePrivacy" <?php if ($user->is_private) echo 'checked'; ?>>
            ...
            <input id="editUserLocation" <?php echo 'data-default-location="' . $user->location . '"'; ?> value={{ $user->location }} type="text" name="location" id="location" maxlength="32">
            ...
            <div class="flex flex-row justify-center w-72 m-auto">
                ...
                <button id="editUserExit" type="button" class="w-36 bg-red-400 hover:bg-red-700 action-button" onclick="closeProfileEditModal();">Exit</button>
            </div>
        </Form>
    </div>
</div>
